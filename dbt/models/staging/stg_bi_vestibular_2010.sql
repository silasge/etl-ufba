...
source as (
    select * from {{ source('socioeconomic', 'BI_Vestibular_2010_Inscritos') }}
),

stage_bi_vestibular_2010 as (
    select
        ano::integer as ano_vestibular,
        inscrica::varchar as codigo_inscricao,
        strptime(dt_nascim, '%Y-%m-%d') as data_nascimento,
...
select * from stage_bi_vestibular_2010
