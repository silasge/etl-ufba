...
source as (
    select * from {{ source('socioeconomic', 'Vestibular_2007_inscritos') }}
),

stage_vestibular_2007 as (
    select
        ano::integer as ano_vestibular,
        inscrica::varchar as codigo_inscricao,
        strptime(nascim, '%d-%b-%Y') as data_nascimento,
...
select * from stage_vestibular_2007
