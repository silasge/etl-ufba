...
source as (
    select * from {{ source('socioeconomic', 'BI_Vestibular_2009_Inscritos') }}
),

stage_bi_vestibular_2009 as (
    select
        ano::integer as ano_vestibular,
        inscrica::varchar as codigo_inscricao,
        strptime(dt_nasci, '%Y-%m-%d') as data_nascimento,
...
select * from stage_bi_vestibular_2009
