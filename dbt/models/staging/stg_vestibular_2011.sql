...
source as (
    select * from {{ source('socioeconomic', 'Vestibular_2011_Inscritos') }}
),

stage_vestibular_2011 as (
    select
        ano::integer as ano_vestibular,
        inscrica::varchar as codigo_inscricao,
        strptime(dt_nasci, '%d.%m.%Y') as data_nascimento,
...
select * from stage_vestibular_2011
