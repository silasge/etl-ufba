...
source as (
    select * from {{ source('socioeconomic', 'BI_Vestibular_2011_Inscritos') }}
),

stage_bi_vestibular_2011 as (
    select
        ...
        strptime(
            regexp_replace(
                ...
                '(/\d{2}\s\d{2}:\d{2}$)',
                '/19'
                || right((regexp_replace(nascim, '(\s\d{2}:\d{2})', '')), 2)
            ),
            '%d/%m/%Y'
        ) as data_nascimento,
...
select * from stage_bi_vestibular_2011
